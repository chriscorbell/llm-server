...
The September 12 FP8 cache comparison passed 18/18 short checks, 3/3 checks at about 90K input and 8/8 Pi xhigh tasks. The service is healthy with FP8 KV at 131,072 tokens and 205,391 KV tokens. Its short checks pass 18/18; validation of a 126K prompt is in progress. Clients remain at 98,304. The committed tuning default still uses FP16; rollback uses `KV_DTYPE=auto MAX_MODEL_LEN_INT4DRAFT=98304`. [Cache comparison](docs/log/2026-09-12-fp8-int4draft.md), [128K experiment](docs/log/2026-09-12-context-128k.md)

The current experiment retains the September 11 campaign's MTP4 default, original pinned engine and unrestricted CPUs. Identical requests confirm a 6.6% warm code-decode gain at 8,240 input tokens, 768 generated tokens, concurrency 1 and thinking off; reasoning throughput is unchanged within the observed spread. [Exact-request confirmation](docs/log/2026-09-11-mtp-fixed-prompt-confirmation.md). The 0.29.0 image and one-CCX CPU pin did not provide material speed gains and were not retained. Pi remains at xhigh by default; `pi --thinking medium` is documented for small edits after two 8/8 task runs at each effort. [Effort comparison](docs/log/2026-09-11-pi-reasoning-effort.md).

Profile `a-int4draft` serves FP8 KV, INT4 draft, MTP4, vision, tool calling and thinking at 131,072 context. The container started September 13 at 01:11:27 UTC, is healthy with zero restarts and holds 205,391 KV tokens. All 64 CPUs remain available. The API is at `http://100.103.136.98:8000/v1`, with its key in the server's private `compose/.env`. The unquantized-draft rollback profile is `a-bf16kv` with `MTP_TOKENS=3`.

The original image remains pinned at `sha256:f01e24f6c7ff01f1e0662234255a1372297d1dbd89d003cf13c8fad3eab1ba4f`. The context experiment keeps utilization 0.95, 8,192 batched tokens and one sequence. XPU graphs were already enabled before tuning. `compose/tuning.env` records the validated defaults; the experiment overrides `KV_DTYPE` and `MAX_MODEL_LEN_INT4DRAFT` in the shell.
...
The running service uses maximum context 131,072 tokens, FP8 KV, GPU utilization 0.95, batched tokens 8,192, one sequence, MTP4, INT4 draft overlay on and prefix caching on. KV capacity at 128K is 205,391 tokens. FP8 at 96K held 191,566; the pre-experiment FP16 baseline had 109,067. The historical `a-bf16kv` profile actually uses `--dtype float16 --kv-cache-dtype auto`; its attention cache follows FP16. The profile name and old log labels are retained for traceability. The example environment now matches the working context and utilization values. [inspection](docs/log/2026-09-07-agent-client-contract.md)
