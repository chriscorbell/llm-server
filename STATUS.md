...
Optimization experiments are in progress on September 11. MTP4 is now the tracked default on the original pinned engine. It passed 18/18 short checks and 8/8 Pi tasks with the [corrected evaluation runner](docs/log/2026-09-11-eval-fixture-escape.md). Repeated MTP3 controls confirm code/tool gains with unchanged cold and cached TTFT. MTP2 was slower and is rejected. [Depth measurements](docs/log/2026-09-11-int4-mtp-depth.md). The current container is healthy with the temporary vLLM 0.29.0 image override (`sha256:1db27a8b75ae1d6b3cbf16ebbb88310c2b5548221c8df1335082b2a54dba209a`), MTP4 and no CPU pinning. The V2 model runner and both INT4 draft conversions are active, with 107,632 KV tokens. The 18/18 short checks pass; throughput and complete-task validation are running. See the separate [vLLM 0.29.0 serving comparison](docs/log/2026-09-11-vllm029-serving.md).
...
- **MTP4 improves code/tool throughput with the INT4 draft.** Compared with the repeated MTP3 control, warm code medians improve 12.0% at about 8.3K input, 8.4% at 32.6K and 3.6% at 56.9K; 8.5K tool output improves 8.8%. All use concurrency 1 and thinking off. Cold and cached TTFT are unchanged. Reasoning results overlap the observed spread. Short checks pass 18/18 and corrected Pi tasks 8/8; no broader quality claim is established. [MTP depth sweep](docs/log/2026-09-11-int4-mtp-depth.md)
...
Tracked validated defaults on the old image: maximum context 98,304 tokens, GPU utilization 0.95, batched tokens 8,192, one sequence, MTP4, INT4 draft overlay on, prefix caching on, 109,067 KV tokens. The historical `a-bf16kv` profile actually uses `--dtype float16 --kv-cache-dtype auto`; its attention cache follows FP16. The profile name and old log labels are retained for traceability. The example environment now matches the working context and utilization values. [inspection](docs/log/2026-09-07-agent-client-contract.md)
...
- **MTP with 4 speculative tokens for the INT4 draft.** Six-repetition cached code medians are 94.1 tok/s at 8,248 actual input tokens, 81.0 at 32,590 and 72.3 at 56,909, thinking off, 768 generated tokens and concurrency 1. The repeated MTP3 controls measured 84.0, 74.7 and 69.8 respectively. Cold 32K TTFT is unchanged at about 18.61 s. Use `MTP_TOKENS=3` with the unquantized `a-bf16kv` rollback profile. [INT4 depth sweep](docs/log/2026-09-11-int4-mtp-depth.md)
