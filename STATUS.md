...
The September 12 FP8 context experiment is in progress. The FP16 baseline passed 18/18 short checks, 3/3 checks at about 90K input tokens and 8/8 Pi xhigh tasks. The service is healthy with FP8 KV at 98,304 and 191,566 KV tokens. Short checks pass 18/18; timing, 90K retrieval and Pi quality comparisons are in progress. Clients remain at 98,304. The committed tuning default still uses FP16; rollback uses `KV_DTYPE=auto MAX_MODEL_LEN_BF16KV=98304`. [Experiment](docs/log/2026-09-12-fp8-int4draft.md)
...
Profile `a-int4draft` serves FP8 KV with the INT4 draft overlay, MTP4, vision, tool calling and thinking at 98,304 context. The container started September 13 at 00:55:48 UTC, is healthy with zero Docker restarts and holds 191,566 KV tokens. All 64 CPUs remain available. The API is at `http://100.103.136.98:8000/v1`, with its key in the server's private `compose/.env`. The unquantized-draft rollback profile is `a-bf16kv` with `MTP_TOKENS=3`.

The original image remains pinned at `sha256:f01e24f6c7ff01f1e0662234255a1372297d1dbd89d003cf13c8fad3eab1ba4f`. The September 11 campaign preserves the 98,304-token limit, utilization 0.95, 8,192 batched tokens and one sequence. XPU graphs were already enabled before tuning. `compose/tuning.env` records the validated defaults; the FP8 experiment overrides only `KV_DTYPE` in the shell.
...
The running service uses maximum context 98,304 tokens, FP8 KV, GPU utilization 0.95, batched tokens 8,192, one sequence, MTP4, INT4 draft overlay on and prefix caching on. FP8 holds 191,566 KV tokens; the pre-experiment FP16 baseline had 109,067. The historical `a-bf16kv` profile actually uses `--dtype float16 --kv-cache-dtype auto`; its attention cache follows FP16. The profile name and old log labels are retained for traceability. The example environment now matches the working context and utilization values. [inspection](docs/log/2026-09-07-agent-client-contract.md)
