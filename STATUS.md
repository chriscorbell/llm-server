...
Profile `a-int4draft` is being recreated with FP8 KV, INT4 draft, MTP4, vision, tool calling and thinking at 131,072 context. Startup and KV capacity will be recorded after initialization. All 64 CPUs remain available. The API is at `http://100.103.136.98:8000/v1`, with its key in the server's private `compose/.env`. The unquantized-draft rollback profile is `a-bf16kv` with `MTP_TOKENS=3`.
...
The current startup uses maximum context 131,072 tokens, FP8 KV, GPU utilization 0.95, batched tokens 8,192, one sequence, MTP4, INT4 draft overlay on and prefix caching on. KV capacity at 128K is pending initialization. FP8 at 96K held 191,566 KV tokens; the pre-experiment FP16 baseline had 109,067. The historical `a-bf16kv` profile actually uses `--dtype float16 --kv-cache-dtype auto`; its attention cache follows FP16. The profile name and old log labels are retained for traceability. The example environment now matches the working context and utilization values. [inspection](docs/log/2026-09-07-agent-client-contract.md)
